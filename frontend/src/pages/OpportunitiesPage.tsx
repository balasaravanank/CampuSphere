...
export default function OpportunitiesPage() {
  ...
  // For staff: check if they have been granted opportunity-posting permission
  const { data: permData } = useQuery({
    ...
    enabled: user?.role === 'staff',  // admin always has access, no need to check
  });

  // Admin always can post; staff only if granted permission
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () =>
      api.get('/opportunities/', { params: { type: filter.toLowerCase() } })
        ...
        <div className="opp-grid">
          {opportunities.map((o, idx) => {
            ...
            return (
              <div
                key={o.id}
                className={`opp-card ${cfg.color} ${o.applied ? 'opp-card-applied' : ''}`}
              ...
              >
                ...
                <div className="opp-card-actions">
                  {/* Apply/Withdraw for students, or View Applicants for staff */}
                  {isStaffOrAdmin ? (
                    <button
                    ...
                    >
                      {applyMutation.isPending ? <Loader2 size={13} className="spin" /> : 'Apply Now'}
                    </button>
                  )}
